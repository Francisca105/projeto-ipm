  ...
  if (draw_targets)
  {
    ...
    if (current_trial === NUM_OF_TRIALS)
    {
      ...
      for (i = 0; i < legendas.getRowCount(); i++){
        if (targets[i].correct == true)
          targets[i].correct = false;
      }
...
function createTargets(target_size, horizontal_gap, vertical_gap)
{
  ...
  for (var r = 0; r < GRID_ROWS; r++)
  {
    for (var c = 0; c < GRID_COLUMNS; c++)
    {
      ...
      let target = new Target(target_x, target_y + 20, target_size, target_label, target_id);
      targets.push(target);
    }  
  }
}
...
function windowResized() 
{
  if (fullscreen())
  {
    ...
    createTargets(target_size * PPCM, horizontal_gap * PPCM - 90, vertical_gap * PPCM - 90);
